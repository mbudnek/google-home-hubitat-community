 ...
 *
 *  Drive does nothing but bridge an HTTP stream into Google Home to display as a video camera.
 *  Saving the preferences creates the video URL link: http://${deviceIP}/h264/${deviceName}/temp.m3u8?user=${deviceUser}&pw=${devicePWD} which will be passed to a Chromecast device
 *  NOTE:  Chromecast requires H.264 video, and AAC audio enabled in the above stream (regardless if the camera source supplies audio)
 *
...
metadata {
    definition (name: "Virtual Camera Stream Object", namespace: "lpakula", author: "Lyle Pakula") {
